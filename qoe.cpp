...
public:
    ...
    Environment_qoe(Info info) : info(info)
    {
    }
...
public:
    ...
    explicit QLearningAgent_qoe(int traffic_length, int num_actions)
...
// 处理每个time槽的函数
void process_time_slot_qoe(int length, int time_slot, Info info, const int episodes, double lossrate)
{
    // 排序
    ...
    // 初始化
    Environment_qoe env(info);
    QLearningAgent_qoe agent(info.num_actions, info.num_actions);
    ...
    string allfile = path + "/allqoe.txt";
    ...
    alloutFile << length <<"\t"<< "QOE\t" <<time_slot<<"\t"<<allqoe/length<<"\t"<<allLoss/success<<"\t"<<allDelay/success<<"\t"<<allRatio/success<<"\t"<<(length-success)/length<<"\t"<<allULRate<<"\t"<<allDLRate<<endl;
    cout << allqoe << endl;
...
void process_time_slot_threaded_qoe(int length, int time_slot, Info info, const int episodes, double lossrate)
{
    process_time_slot_qoe(length, time_slot, info, episodes, lossrate);
}

void appendFileContent_qoe(const filesystem::path &sourceFile, std::ofstream &targetFile)
{
...
void uniontxt()
{
    ...
    std::string dirPath = current_path.string(); // 当前目录，你可以修改为需要的目录路径
    cout << dirPath << endl;
    for (const auto &entry : filesystem::recursive_directory_iterator(dirPath))
    {
        if (entry.is_regular_file() && entry.path().extension() == ".txt")
        {
            ...
            appendFileContent_qoe(entry.path(), targetFile);
        }
    }
...
void calculate_all_qoe(filesystem::path current_path, int len,int choice,int plus)
{
    for (int length = len; length < len+plus && length<=2000; length += 50)
    {
        int episodes = (length>=1500)?13000:(length >= 800) ? 10000 : (length > 600) ? 6000
                                                                : 4000;
        ...
        for (int i = range[choice][0]; i <range[choice][1]; i++)
        {
            ...
            for (int time = times[i][0]; time < times[i][1]; ++time)
                threads.emplace_back(process_time_slot_qoe, length, time, info, episodes, lossrate);
